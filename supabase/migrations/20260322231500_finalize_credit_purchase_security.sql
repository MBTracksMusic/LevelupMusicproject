/*
  # Finalize secure credit purchases

  Goals:
  - keep products.price as the single pricing source
  - lock 1 credit = 10 EUR = 1000 cents in the RPC itself
  - keep the legacy second argument for compatibility, but ignore it
  - enforce balance checks and entitlement granting atomically
*/
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'version', 'credits_v4_fixed_unit_value'
  )
)
...
COMMENT ON FUNCTION public.purchase_beat_with_credits(uuid, uuid) IS
  'Atomic beat purchase with credits. Credits required are derived from products.price using a fixed 10 EUR per credit value; the legacy second argument is ignored.';

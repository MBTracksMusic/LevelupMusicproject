/*
  # Use campaign submission_deadline as battle voting end on launch

  Goal:
  - When admin clicks Launch Battle on an official campaign,
    submission_deadline becomes the actual battle end datetime (voting_ends_at).

  Impact:
  - Future launches: voting_ends_at is set to campaign submission_deadline.
  - Existing in-progress admin campaign battles: voting_ends_at is aligned to campaign submission_deadline.
*/
...
-- Backfill already launched in-progress admin campaign battles.
UPDATE public.battles b
SET voting_ends_at = c.submission_deadline,
    updated_at = now()
FROM public.admin_battle_campaigns c
WHERE c.battle_id = b.id
  AND b.battle_type = 'admin'
  AND b.status IN ('active', 'voting', 'awaiting_admin', 'approved', 'pending_acceptance')
  AND c.submission_deadline > now()
  AND b.voting_ends_at IS DISTINCT FROM c.submission_deadline;

...
const maskEmail = (email) => {
    ...
    const local = localRaw.length <= 2 ? `${localRaw.slice(0, 1)}*` : `${localRaw.slice(0, 2)}***`;
...
const buildEmailLockMarker = (nowMs) => new Date(nowMs + emailLockMarkerOffsetMs).toISOString();
const getEmailLockClaimStartedAtMs = (value) => {
    const parsedMs = parseIsoMillis(value);
    if (parsedMs === null)
        return null;
    if (new Date(parsedMs).getUTCFullYear() < emailLockMarkerYearThreshold)
        return null;
...
const isRetryableNotificationError = (error) => {
...
const extractStoragePathFromCandidate = (candidate, bucket) => {
    if (!candidate)
        return null;
...
const formatNumberLimit = (value) => value === null ? "Illimite" : value.toLocaleString("fr-FR");
const buildLicensePresentation = (license, fallbackLicenseName, producerName) => {
    const resolvedName = (license?.name || fallbackLicenseName || "Standard").trim();
    const resolvedDescription = license?.description ||
...
async function createSignedUrl(bucket, rawPath, options) {
...
async function downloadContractPdfFromStorage(bucket, rawPath) {
    const extractedPath = extractStoragePathFromCandidate(rawPath, bucket);
    const sourcePath = extractedPath || normalizeStoragePath(rawPath);
    if (!sourcePath) {
        return { buffer: null, resolvedPath: null, error: "invalid_storage_path" };
    }
    const pathCandidates = buildStoragePathCandidates(sourcePath, bucket);
    let lastError = null;
    for (const pathCandidate of pathCandidates) {
...
app.post("/generate-contract", async (req, res) => {
    ...
    try {
      ...
      `)
        ...
        const licensePresentation = buildLicensePresentation(purchase.license, fallbackLicenseName, producerName);
        const pdfPayload = {
            purchaseId,
            contractDate,
            producerName,
...
app.all("/generate-contract", (_req, res) => {
